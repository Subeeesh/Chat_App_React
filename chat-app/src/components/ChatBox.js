...
const ChatBox = ({ chats, setChats, activeChatId, updateChatName}) => {
    ...
    console.log("ChatBox: Rendering with activeChatId:", activeChatId, "activeChat:", activeChat);
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            if (!activeChat?.messages || activeChat.messages.length === 0 || activeChat.name.startsWith("New Chat")) {
                const nameUpdateResponse = await fetch(
                    `http://localhost:5000/api/chats/${activeChatId}/update-name`,
                    {
                        method: "PUT",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ chatId: activeChatId, sender: "user", message: input }),
                    }
                );
    
                if (!nameUpdateResponse.ok) {
                    const errorData = await nameUpdateResponse.json();
                    throw new Error(`HTTP error! status: ${nameUpdateResponse.status}, details: ${JSON.stringify(errorData)}`);
                }
            ...
            if (!messageResponse.ok) {
                ...
                throw new Error(`HTTP error! status: ${messageResponse.status}, details: ${JSON.stringify(errorData)}`);
            }
    ...
    if (!activeChat) {
        return <div className="flex-1 h-screen bg-gray-800 text-white p-4">No chat selected.</div>;
    }
    ...
    return (
        <div className="flex flex-col flex-1 h-screen bg-gray-800 text-white">
            ...
            <div className="p-4 bg-gray-900 flex items-center">
                ...
                />
                <button className="ml-2 px-4 py-2 bg-blue-500 rounded-lg" onClick={sendMessage}>
                    Send
                </button>
            </div>
        </div>
